...
class ProyectosFirebase {
  ...
  Stream<List<QueryDocumentSnapshot>> getAllDonations() {
    ...
    final childReferences = parentSnapshot.docs.map((doc) => doc.reference.collection('donaciones'));
    final childStreams = childReferences.map((childReference) => childReference.where('correo', isEqualTo: FirebaseAuth.instance.currentUser!.email).snapshots());
